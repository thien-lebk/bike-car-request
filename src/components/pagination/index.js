...
const Component = (
  {
    ...
    queryParams = () => {},perPageLable=["of","items","page"]
  }
) => {
    ...
    <div className="flex items-center justify-between mt-3 select-none">
      <div>
        <Select defaultValue={pageSize} className="min-w-[115px]" onChange={(value) => onPageSizeChange(value)}>
          {pageSizeOptions.map((item, index) => <Select.Option key={index} value={item}>{item} / {perPageLable[2]}</Select.Option>)}
        </Select>
      ...
      <div className="flex justify-center border border-gray-100 p-1 rounded-xl bg-white">
        <div className="flex flex-wrap justify-center duration-300 transition-all">
          {listOfPageItem.current.map((page, index) => (
            <button
              key={index}
              className={classNames('text-center duration-300 transition-all py-1 px-3 text-sm font-medium leading-normal', {
            ...
            >
              {page.type === 'prev' && <span className="uhome-first_page p-1 border-[1.5px] border-blue-700 text-blue rounded-full text-xl"></span>}
              {page.type === 'next' && <span className="uhome-next_page p-1 border-[1.5px] border-blue-700 text-blue rounded-full text-xl"></span>}
