...
import Timeline,  { TimelineHeaders, SidebarHeader, DateHeader, TimelineMarkers, CustomMarker } from 'react-calendar-timeline';
...
const Component = (
  ...
  const [groups, set_groups] = useState([{id: 1, title: 'group 1', root: true, parent: null,hasChildren: true,levelChildren: 0}]);
  const [items, set_items] = useState([{id: 1, group: 1, title: 'item 1', start: moment(), end: moment().add(1, 'hour')}]);
  ...
  const toggleGroup = (id) => {
    if (!openGroups[id]) {
      ...
      for (let key in openGroups) {
        if (openGroups.hasOwnProperty(key) && key.indexOf(id) > -1) {
  ...
  const newGroups = groups
    ...
    .map(group => {
      return Object.assign({}, group, {
        title: !!isGroup && group.hasChildren ? (
          <div onClick={() => toggleGroup(group.id)} style={{ cursor: "pointer", paddingLeft: (group.levelChildren * 15) }}>
            {openGroups[group.id] ? <i className="las la-angle-down" /> : <i className="las la-angle-right" />} {group.title}
          </div>
        ) : (
  ...
  const itemRenderer = ({ item, itemContext, getItemProps, getResizeProps }) => {
    ...
    const backgroundColor = itemContext.selected ? (itemContext.dragging ? "red" : selectedColor) : (item.bgColor || primaryColor);
    ...
    <Spin spinning={isLoading}>
      ...
      >
        <TimelineHeaders className="sticky">
          <SidebarHeader>
            {({ getRootProps }) => {
              return <div {...getRootProps()} className="title-left">{title}</div>;
            }}
          </SidebarHeader>
