...
const Component = (
  ...
  const [handleEditForm, ModalForm] = HookModalForm({
    ...
    isLoading, setIsLoading, columns, Post, Put, GetById, widthModal: widthForm,
    idElement,
    handleChange: (values, oldData) => handleSubmit(values, oldData?.id), ...propForm
  });

  const renderRow = (item, collapseIcon) =>item&& (!condition || condition(item)) && (
    <div className="item-drag flex justify-between items-center" key={item?.id}>
      ...
      {!readOnly && (
        <div className={classNames('pr-1 flex items-center justify-end pt-1',{'w-20': !!conditionDelete(item), 'w-10': !conditionDelete(item)})}>
          ...
          {(((!!allowActions && !item?.allowActions) || !!item?.allowActions?.allowDelete || (typeof item?.id === 'string' && item?.id?.length === 40)) && conditionDelete(item)) && (
            <Tooltip title={t("routes.admin.Layout.Delete")}>
              <Popconfirm
                onVisibleChange={(visible) => changeTitleConfirmDelete && changeTitleConfirmDelete(visible, item, set_titleConfirmDelete)}
  ...
  return !!items ? (
    <Fragment>
      <div className="flex justify-between items-center mb-3">
        <div>
          {renderLeftButton && renderLeftButton(items)}
        </div>
          ...
          {!readOnly && !!showAddNew && (
            <button
              ...
              onClick={() => {
                ...
                // console.log("initAddNew",initAddNew)
              }}
            >
