...
const Component = ({ formItem, placeholder, onChange, value, form,className }) => {
  ...
  return (
    <TreeSelect
      ...
      dropdownRender={(originNode) => (
        <Fragment>
          {formItem.mode === "multiple" && <Fragment>
            <Checkbox checked={checkAll} onChange={() => onChange(allValue.current)}>Select all</Checkbox>
            <Checkbox checked={unCheckAll} onChange={() => onChange([])}>Deselect all</Checkbox>
          </Fragment>}
          {originNode}
        </Fragment>
      )}
