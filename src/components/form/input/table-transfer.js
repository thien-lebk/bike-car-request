...
const Component = ({showSearch = true, formItem, onChange, value, form, ...restProps}) => {
  ...
  return (
    <Transfer
      ...
      filterOption={(inputValue, item) => !!formItem.transfer.filter ? formItem.transfer.filter(inputValue, item) : item.fullName.toUpperCase().indexOf(inputValue.toUpperCase()) !== -1}
    >
      {({
          direction,
          filteredItems,
