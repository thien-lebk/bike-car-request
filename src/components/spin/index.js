...
const Component = ({spinning = true, text = "Now Loading...", children, className = ""}) => {
  return (
    <div className={classNames("relative min-h-[80px]")}>
      <div
        className={classNames("w-full h-full flex-col absolute top-0 left-0 right-0 bottom-0 z-10 mx-0 flex justify-center items-center transition-all ease-in-out duration-300", {
          'invisible opacity-0': !spinning, 'visible opacity-100': spinning}
        )} >
